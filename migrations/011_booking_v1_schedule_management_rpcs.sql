...
-- Allow owner/admin to manage any staff schedules.
-- Optionally allow staff to manage their own schedules (set v_allow_staff_self_manage = true).
...
begin
  ...
  -- Role gate: owner/admin; optionally staff for their own
  ...
  if p_rule_id is null then
    ...
    -- Ensure rule belongs to org and (if staff) belongs to them
    if not exists (
      select 1
