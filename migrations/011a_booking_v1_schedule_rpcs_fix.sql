...
-- Drop the broken functions (if they exist)
drop function if exists pods.rpc_upsert_availability_rule_v1(
  uuid, uuid, uuid, uuid, int, time, time, boolean
);

drop function if exists pods.rpc_delete_availability_rule_v1(uuid, uuid);

-- Recreate with valid parameter ordering (NO defaults in the middle)
-- Callers pass NULL explicitly for p_rule_id when creating.
create or replace function pods.rpc_upsert_availability_rule_v1(
  p_org_id uuid,
  p_rule_id uuid,                -- pass NULL to create
