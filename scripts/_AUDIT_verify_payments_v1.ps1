...
$out = Join-Path $RepoRoot ("proofs\audit\verify_payments_" + $ts + ".txt")
$C = "supabase_db_proteusops"
function Log($m){ Add-Content -LiteralPath $out -Value $m }
Set-Content -LiteralPath $out -Value ("ProteusOps VERIFY PAYMENTS (LOCAL) " + $ts)
Log "`n===== supabase db reset (full chain incl payment idempotency) ====="
...
\echo :::PAYMENT_LIFECYCLE (existing):::
select pods_provisioning.rpc_selftest_payment_lifecycle_v1()->>'ok' as lifecycle_ok;
\echo :::SECURITY REGRESSION:::
select pods_core.rpc_selftest_rls_fail_closed_v1()->>'token' as rls, pods_core.rpc_selftest_session_assurance_v1()->>'token' as sess, pods_provisioning.rpc_verify_platform_constitution_v1()->>'ok' as constitution_ok;
'@
