...
$out = Join-Path $RepoRoot ("proofs\audit\verify_slice45_" + $ts + ".txt")
$C = "supabase_db_proteusops"
function Log($m){ Add-Content -LiteralPath $out -Value $m }
Set-Content -LiteralPath $out -Value ("ProteusOps VERIFY slices 4+5 (LOCAL) " + $ts)
Log "`n===== supabase db reset (full chain) ====="
