...
$out = Join-Path $RepoRoot ("proofs\audit\verify_slice7_" + $ts + ".txt")
$C = "supabase_db_proteusops"
function Log($m){ Add-Content -LiteralPath $out -Value $m }
Set-Content -LiteralPath $out -Value ("ProteusOps VERIFY slice 7 (6g grace + H1 hub) (LOCAL) " + $ts)
...
select pods.rpc_selftest_subscription_lapse_v1()->>'token' as lapse;
select pods_provisioning.rpc_selftest_stripe_ingest_wrappers_v1()->>'token' as wrappers;
