...
const emailRaw = process.env.TEST_EMAIL;
const passwordRaw = process.env.TEST_PASSWORD;
const ORG_ID_RAW = process.env.ORG_ID;
...
async function main() {
  const url = req("SUPABASE_URL", urlRaw).replace(/\/+$/, "");
  const anon = req("SUPABASE_ANON_KEY", anonRaw);
  const service = req("SUPABASE_SERVICE_ROLE_KEY", serviceRaw);
  const email = req("TEST_EMAIL", emailRaw);
  const password = req("TEST_PASSWORD", passwordRaw);
  const ORG_ID = req("ORG_ID", ORG_ID_RAW);
